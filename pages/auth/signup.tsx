...
import CircledIconBtn from "@/components/CircledIconButton";
...
export default function SignUp({ providers }: SignInProps) {
  ...
  const signUpHandler = async () => {
    try {
      ...
      setLoading(true);
      if (picture) {
        const uploaded_picture = await uploadFiles([{ file: picture, type: "image" }]);
      ...
      <div className="relative min-h-screen bg-gray-50 dark:bg-gray-900 flex justify-center items-center">
        <div className="bg-white dark:bg-gray-800 shadow-xl rounded-lg p-8 w-full max-w-md">
          <div className="flex items-center justify-between mb-6">
            <div
              className="w-10 h-10 border border-gray-300 dark:border-gray-600 rounded-full flex items-center justify-center cursor-pointer hover:border-blue-500"
              onClick={() => router.push("/")}
            >
              <BiLeftArrowAlt className="text-gray-600 dark:text-gray-300 hover:text-blue-500" />
            </div>
          ...
          <h1 className="text-2xl font-bold text-gray-800 dark:text-gray-100 mb-4">Sign Up</h1>
          ...
          <div className="mt-6">
            <span className="block text-center text-gray-500 dark:text-gray-300 mb-4">
              Or continue with
            </span>
            <div className="flex flex-col gap-4">
              {providers.map((provider) => {
                ...
                return (
                  <button
                    key={provider.name}
                    className="w-full flex items-center justify-center border border-gray-300 dark:border-gray-600 rounded-lg py-2 hover:bg-gray-100 dark:hover:bg-gray-700"
